...
class gsp_icbs_loan_loanLossloanDeferredCredit_gsp extends GroovyPage {
public String getGroovyPageFileName() { "/WEB-INF/grails-app/views/loan/loanLoss/loanDeferredCredit.gsp" }
public Object run() {
Writer out = getOut()
...
printHtmlPart(14)
invokeTag('select','g',56,['id':("txnType"),'name':("txnType"),'from':(icbs.admin.TxnTemplate.findAllByTxnTypeAndMemoTxnType(icbs.lov.TxnType.read(64),icbs.lov.MemoTxnType.read(1))),'optionKey':("id"),'optionValue':("description"),'required':(""),'value':(""),'onchange':("getcode();"),'class':("many-to-one form-control")],-1)
printHtmlPart(15)
invokeTag('field','g',62,['class':("form-control truncated"),'id':("creditAmt"),'name':("creditAmt"),'value':("")],-1)
printHtmlPart(16)
invokeTag('hiddenField','g',65,['name':("deferredCreditId"),'id':("deferredCreditId"),'value':(params.id)],-1)
printHtmlPart(17)
...
invokeTag('form','g',80,['id':("deposit"),'url':([controller:loan, action:'deferredCredit']),'method':("PUT")],3)
printHtmlPart(20)
})
