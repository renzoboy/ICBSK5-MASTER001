...
printHtmlPart(2)
invokeTag('select','g',7,['id':("products"),'name':("products"),'from':(icbs.admin.Product.findAll("from Product where product_type_id in (6,7) and config_item_status_id = 2")),'optionKey':("id"),'optionValue':("name"),'value':(loanDeductionSchemeInstance?.products?.id),'class':("many-to-one form-control"),'multiple':("")],-1)
printHtmlPart(3)
}
