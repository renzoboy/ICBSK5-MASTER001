...
<html>
	<head>
		...
		<g:if test="${module == "loanRestructure"}">
                    ...
                    <title>Restructure Account</title>
                </g:if>
                <g:else>
                   
                    <title>Reschedule Account</title>
                </g:else>  		
		<g:javascript>
                                function deduct() 
                                {
	...
	<body>
        <content tag="breadcrumbs">
            <span class="fa fa-chevron-right"></span><span class="current">Restructure/Reschedule Account</span>
	</content>
		...
		<div id="edit-loan" class="content scaffold-edit" role="main">
			...
			<g:form id="loan-form" url="[resource:loanInstance, action:'update']" method="PUT" 
				...
				<g:hiddenField name="version" value="${loanInstance?.version}" />
                                <g:if test="${module == "loanReschedule" && ((loanInstance?.loanPerformanceId.id == 1 || loanInstance?.loanPerformanceId.id == 5 || loanInstance?.loanPerformanceId.id == 6) && loanInstance?.status?.id == 4)}">
                                    <g:hiddenField name="activity" value="Reschedule" />
                                </g:if>
                                <g:else>
                                    <g:hiddenField name="activity" value="Restructure" />
                                </g:else>    
        ...
        <content tag="main-actions">
            <ul>
                ...
                <li><a href="#" onclick="restructureCallAlertify();">Save Restructure</a></li>
                </g:if>   
                <g:elseif test="${module == "loanReschedule" && ((loanInstance?.loanPerformanceId.id == 1 || loanInstance?.loanPerformanceId.id == 5 || loanInstance?.loanPerformanceId.id == 6 ) && loanInstance?.status?.id == 4)}">
               
                    <li><a href="#" onclick="rescheduleCallAlertify();" >Save Reschedule</a></li>    
                    
                </g:elseif>
                
                <li><a href="#" onclick="redirectToShowLoanRescheduleOrRestructured();" >Cancel</a></li>
                <script>
                    function rescheduleCallAlertify(){
                        
                      alertify.confirm(AppTitle,"Are you sure you want to Reschedule this Account?",
                            function(){
                              checkIfAllowed('LON01300', 'form#loan-form', 'Update branch XXX.', null); // params: policyTemplate.code, form to be saved
                            },
                            function(){
                              alertify.error('Account Reschedule Canceled!');
                            });
                    }
                    function redirectToShowLoanRescheduleOrRestructured(){
                        
                        var urlToLoad = '/icbs/'+'${module}'+'/show/'+${loanInstance.id};
                        console.log("urlToLoad: "+urlToLoad);
                        alertify.confirm(AppTitle,"Are you sure you want to cancel this process ?",
                            function(){
                    ...
                    function restructureCallAlertify(){
                        alertify.confirm(AppTitle,"Are you sure you want to Restructure this Account?",
                            function(){
                              checkIfAllowed('LON01400', 'form#loan-form', 'Update branch XXX.', null); // params: policyTemplate.code, form to be saved
                            },
                            function(){
                              alertify.error('Account Restructure Canceled!');
                            });
