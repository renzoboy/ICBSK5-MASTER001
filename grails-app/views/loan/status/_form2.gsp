        ...
        <div class="form-group fieldcontain ${hasErrors(bean: loanInstance, field: 'status', 'has-error')} ">
            ...
            <div class="col-sm-8">
                <g:if test="${loanInstance?.balanceAmount == 0 && loanInstance?.interestBalanceAmount ==  0 &&
                    ...
                    <g:select id="status" name="status.id" from="${LoanAcctStatus.list().findAll{it.id == 6}}" optionKey="id" optionValue="description" value="${loanInstance?.status?.id}" class="form-control"/>
                </g:if>
