...
<html>
	<head>
		...
		<title>Override Log</title>
	</head>
	<body>
            <content tag="breadcrumbs">
		 <span class="fa fa-chevron-right"></span><span class="current">Override Log</span>
	    </content>
			...
			<div class="table-responsive">
				...
				<g:each in="${auditLogInstanceList}" status="i" var="auditLogInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>
							<g:if test="${auditLogInstance.recordUrl}">
							<a href="${fieldValue(bean: auditLogInstance, field: "recordUrl")}" target="_blank">${fieldValue(bean: auditLogInstance, field: "module.name")}</a>
							</g:if>
							${auditLogInstance.description}
						</td>
