...
<div class="fieldcontain form-group ${hasErrors(bean: loanApplicationInstance, field: 'product', 'has-error')}">
	...
	<div class="col-sm-8"><g:select id="product" name="product.id" from="${icbs.admin.Product.findAll("from Product where product_type_id = 6 and config_item_status_id = 2")}" optionKey="id" optionValue="name" value="${loanApplicationInstance?.product?.id}" onclick="loanProduct();" class="many-to-one form-control" noSelection="${['null':'Choose Product']}"/> 
...
<div class="fieldcontain form-group ${hasErrors(bean: loanApplicationInstance, field: 'guaranteeFacility', 'has-error')}">
	...
	<div class="col-sm-8"><g:select id="guaranteeFacility" name="guaranteeFacility" from="${icbs.lov.LoanGuaranteeFacility.list(sort: "code",order: "asc")}" optionKey="id" optionValue="description" value="${loanApplicationSpecAdd?.guaranteeFacility?.id}" class="many-to-one form-control"/>
