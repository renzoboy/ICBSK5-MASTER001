...
private renderFieldForProperty(p, owningClass, prefix = "") {
	...
	boolean required = false
        def it;//para sa custom rendering ng errors
	if (hasHibernate) {
...
<div class="fieldcontain form-group \${hasErrors(bean: ${propertyName}, field: '${prefix}${p.name}', 'has-error')} ${required ? 'required' : ''}">
	<label class="control-label col-sm-4" for="${prefix}${p.name}">
		<g:message code="${domainClass.propertyName}.${prefix}${p.name}.label" default="${p.naturalName}" />
		<% if (required) { %><span class="required-indicator">*</span><% } %>
	</label>
	<div class="col-sm-8">${renderEditor(p)}
